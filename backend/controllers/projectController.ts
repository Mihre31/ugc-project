...
export const createProject = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const prompt = {
      text: `Combine the person and the product into a realistic photo.
       Make the person naturally hold or use the product.
      Match lighting, shadows, scale and perspective. Make the person stand in
      professional studio lighting. Output ecommerce-quality photo realistic imagery ${userPrompt}`,
    };
    ...
    const uploadResult = await cloundinary.uploader.upload(base64Image, {
      resource_type: "image",
    });
...
export const createVideo = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (!operation.response.generatedVideos) {
      throw new Error(
        operation.response.raiMediaFilteredReasons[0] ||
          "Video generation failed or no video was returned",
      );
    }
    ...
    const uploadResult = await cloundinary.uploader.upload(filePath, {
      resource_type: "video",
    });
    ...
    //Remove video file from desk after upload
